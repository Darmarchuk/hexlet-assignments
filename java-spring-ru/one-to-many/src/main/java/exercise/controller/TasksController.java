...
public class TasksController {
    ...
    @PutMapping("/{id}")
    public void update(@PathVariable Long id, @RequestBody TaskUpdateDTO dto){
        User user=null;
        Task task=taskRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("Task not found"));
        if(task.getAssignee().getId()!=dto.getAssigneeId()  )
            user= userRepository.findById(dto.getAssigneeId()).get();
